...
IMPLEMENT ADDITIONAL TYPE RESTRICTIONS FOR THINGS THAT CAN'T BE USED ON EVERYTHING
  (LIKE IF_GATE' WHERE TYPE VARIABLE U SHOULD ACTUALLY BE RESTRICTED TO QUANTUM STRUCTURES)?
...
ADD OPTION OF INTERPRETING NON-QUANTUM EXPRESSIONS ON COMMAND LINE?
TYPE OPERATORS LIKE * AND \/
COMPILE-TIME WARNINGS FOR CRASH
CRASH WITH AN ERROR MESSAGE (WARNING-FREE)
...
REMOVE DATA.MAP QUALIFIER IF I END UP NOT USING DATA.SET
